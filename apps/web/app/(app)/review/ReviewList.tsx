...
export function ReviewList({ needsReview, confirmed, rejected }: Props) {
  ...
  return (
    <div>
      {/* ── Tab bar ───────────────────────────────────────────────────────── */}
      <div className="sticky top-14 z-40 -mx-4 sm:-mx-6 bg-white dark:bg-gray-900">
        {/* Tab row */}
        <div className="mx-4 sm:mx-6 flex gap-1 overflow-x-auto border-b border-gray-200 dark:border-gray-700">
          {tabs.map((tab) => (
            <button
              key={tab.id}
        ...
        {/* Sub-tab action row — sits below the border line */}
        {activeTab === "evidence_ready" && evidenceReady.length > 0 && (
          <div className="mx-4 sm:mx-6 flex justify-end py-2">
            <a
              href="/export"
              className="rounded-md bg-violet-600 px-4 py-1.5 text-xs font-semibold text-white hover:bg-violet-700"
            >
              Go to Export →
            </a>
          </div>
