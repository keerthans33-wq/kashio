...
import {
  confirmWithDetails, rejectCandidate, maybeCandidate, resetCandidate,
...
const SUGGESTION_COLOR: Record<SuggestionLevel, string> = {
  LIKELY_WORK_RELATED:   "#22C55E",
  POSSIBLE_WORK_RELATED: "#14B8A6",
  PROBABLY_PERSONAL:     "#6B7280",
};

const SUGGESTION_LABEL: Record<SuggestionLevel, string> = {
  LIKELY_WORK_RELATED:   "Suggested",
  POSSIBLE_WORK_RELATED: "Possible",
  PROBABLY_PERSONAL:     "Personal",
};

const CARD_BG: Record<Status, string> = {
  CONFIRMED:    "rgba(17, 33, 24, 0.78)",
  REJECTED:     "rgba(17, 24, 39, 0.38)",
  MAYBE:        "rgba(30, 25, 15, 0.78)",
...
export function TransactionCard({
  ...
  const handleMaybe  = () => save(() => maybeCandidate(id),           "MAYBE");
  const handleReset  = () => save(() => resetCandidate(id),           "NEEDS_REVIEW");

  const accentColor = settled
    ? (status === "CONFIRMED" ? "#22C55E" : status === "MAYBE" ? "#F59E0B" : "transparent")
    : SUGGESTION_COLOR[suggestionLevel];

  const suggColor = SUGGESTION_COLOR[suggestionLevel];
  ...
  return (
    ...
    >
      ...
      {/* Main content */}
      <div className="px-5 py-4" style={{ opacity: status === "REJECTED" ? 0.5 : 1 }}>

        {/* Row 1: merchant + amount */}
        <div className="flex items-center justify-between gap-3">
          <p className="truncate text-[14px] font-medium" style={{ color: "var(--text-secondary)" }}>
            {transaction.normalizedMerchant}
          </p>
          <p
            className="shrink-0 text-[22px] font-bold tabular-nums tracking-tight leading-none"
            style={{ color: status === "CONFIRMED" ? "#22C55E" : status === "MAYBE" ? "#F59E0B" : "var(--text-primary)" }}
          >
            ${Math.abs(transaction.amount).toFixed(2)}
          </p>
        </div>

        {/* Row 2: date · suggestion badge · category */}
          ...
          {!settled && (
            <>
              ...
              <span
                ...
                style={{ color: suggColor }}
              >
                <span className="inline-block h-1.5 w-1.5 rounded-full shrink-0" style={{ backgroundColor: suggColor }} />
                {SUGGESTION_LABEL[suggestionLevel]}
              </span>
            </>
          )}
          <span className="text-[12px]" style={{ color: "rgba(255,255,255,0.12)" }}>·</span>
        ...
        {/* Row 3: reason (single line) */}
        {!settled && (
          <p className="mt-1.5 text-[12px] leading-snug line-clamp-1" style={{ color: "var(--text-muted)" }}>
            {reason}
          </p>
        )}
          ...
          {settled ? (
            ...
            <div className="flex items-center justify-between gap-2">
              <span className="text-[12px] font-medium" style={{
                color: status === "CONFIRMED" ? "#22C55E" : status === "MAYBE" ? "#F59E0B" : "var(--text-muted)",
              }}>
                {status === "CONFIRMED" ? (workPct !== "" ? `✓ Claimed · ${workPct}% work` : "✓ Claimed")
                 : status === "MAYBE"    ? "◎ Maybe"
                 : "Ignored"}
              </span>
            ...
            <div className="flex items-center gap-2">
              ...
              <Button
                ...
                onClick={handleMaybe}
                disabled={isSaving}
                style={{ backgroundColor: "rgba(245,158,11,0.10)", border: "1px solid rgba(245,158,11,0.22)", color: "#F59E0B" }}
              >
                Maybe
              </Button>
      ...
      <AnimatePresence>
        {expanded && (
          ...
          >
            <div className="px-5 py-4 space-y-4" style={{ borderTop: "1px solid rgba(255,255,255,0.06)" }}>
              ...
              <div>
                ...
                <p className="text-[12px] leading-relaxed break-words font-mono" style={{ color: "var(--text-muted)" }}>
                  {transaction.description}
                </p>
              </div>

              {/* Work-use split — only when claimed */}
              {status === "CONFIRMED" && (
                <div>
                  ...
                  <div className="flex items-center gap-3">
                    <div className="relative flex-1 max-w-[140px]">
                      <input
                        ...
                        style={{ backgroundColor: "rgba(255,255,255,0.05)", border: "1px solid rgba(255,255,255,0.08)", color: "var(--text-primary)" }}
                      />
                      <span className="pointer-events-none absolute right-3 top-1/2 -translate-y-1/2 text-[13px]" style={{ color: "var(--text-muted)" }}>%</span>
                    </div>
              ...
              {status === "CONFIRMED" && (
                <div>
                  ...
                  {evidenceSaving && <p className="mt-1.5 text-[12px]" style={{ color: "var(--text-muted)" }}>Saving…</p>}
                  {evidenceSaved && !evidenceSaving && <p className="mt-1.5 text-[12px]" style={{ color: "#22C55E" }}>{evidenceSaved}</p>}
                </div>
              )}
